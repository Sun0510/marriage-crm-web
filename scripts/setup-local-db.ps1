...
$allInOneSqlPath = Join-Path $projectPath "database\yeonsu-crm-all-in-one.sql"
...
try {
    $sql = (Get-Content -Raw -Encoding UTF8 -LiteralPath $allInOneSqlPath).Replace("P@ssw0rd", $AppPassword.Replace("'", "''"))
